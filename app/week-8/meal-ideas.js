...
const MealIdeas = ({ ingredient }) => {
    ...
    const [expandedMealId, setExpandedMealId] = useState(null); // State to track the currently expanded meal
    ...
    const toggleIngredients = (mealId) => {
        setExpandedMealId(prevId => (prevId === mealId ? null : mealId)); // Toggle the meal ID
    };
                        ...
                        {expandedMealId === meal.idMeal && mealDetails[meal.idMeal] && (
                            <ul className="mt-2 text-sm text-gray-200">
                                {Object.entries(mealDetails[meal.idMeal])
                                    .filter(([key, value]) => key.startsWith('strIngredient') && value) // Filter out only ingredients
                                    .map(([key, value], index) => (
                                        <li key={index} className="flex justify-between">
                                            <span>{value}</span>
                                            <span>{mealDetails[meal.idMeal][`strMeasure${key.slice(-1)}`]}</span> {/* Measure of each ingredient */}
                                        </li>
                                    ))}
                            </ul>
                        )}
