...
export default function NewItem({ onAddItem }) { // Accept onAddItem as a prop
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-2 shadow rounded-lg w-full max-w-md mb-4">
      ...
      <div className="flex items-center mb-2"> {/* Adjusted for spacing */}
        <div className="flex items-center flex-grow mr-2"> {/* Added margin for gap */}
          <button
            type="button"
          ...
          <span className="text-base font-semibold flex-grow text-center text-black">{quantity}</span> {/* Changed color to black */}
          <button
            type="button"
        ...
        <select
          ...
          className="border p-1 text-black rounded-md flex-grow" // Ensure it grows to fill space
        >
